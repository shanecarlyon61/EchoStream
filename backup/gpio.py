...
def gpio_monitor_worker(arg=None):
    ...
    if not (
        init_gpio_pin(gpio_pin_38)
        and init_gpio_pin(gpio_pin_40)
        and init_gpio_pin(gpio_pin_16)
        and init_gpio_pin(gpio_pin_18)
    ):
    ...
    if gpio_38_state == 0:
        for i in range(4):
            if (
                audio.channels[i].active
                and audio.channels[i].audio.channel_id == global_channel_ids[0]
            ):
                audio.channels[i].audio.gpio_active = 1
                print(
                    f"Channel {global_channel_ids[0]} audio ENABLED (PIN 38 was already active)"
                )
                break
    ...
    if gpio_40_state == 0:
        for i in range(4):
            if (
                audio.channels[i].active
                and audio.channels[i].audio.channel_id == global_channel_ids[1]
            ):
                audio.channels[i].audio.gpio_active = 1
                print(
                    f"Channel {global_channel_ids[1]} audio ENABLED (PIN 40 was already active)"
                )
                break
    ...
    if gpio_16_state == 0:
        for i in range(4):
            if (
                audio.channels[i].active
                and audio.channels[i].audio.channel_id == global_channel_ids[2]
            ):
                audio.channels[i].audio.gpio_active = 1
                print(
                    f"Channel {global_channel_ids[2]} audio ENABLED (PIN 16 was already active)"
                )
                break
    ...
    if gpio_18_state == 0 and global_channel_count > 3:
        for i in range(4):
            if (
                audio.channels[i].active
                and audio.channels[i].audio.channel_id == global_channel_ids[3]
            ):
                audio.channels[i].audio.gpio_active = 1
                print(
                    f"Channel {global_channel_ids[3]} audio ENABLED (PIN 18 was already active)"
                )
                break
        ...
        for i in range(4):
            if (
                ...
                if not audio.channels[i].audio.gpio_active:
                    audio.channels[i].audio.gpio_active = 1
                    print(
                        f"[GPIO] Channel {global_channel_ids[0]} audio ENABLED (re-sync: PIN 38 is active)"
                    )
        ...
        for i in range(4):
            if (
                ...
                if not audio.channels[i].audio.gpio_active:
                    audio.channels[i].audio.gpio_active = 1
                    print(
                        f"[GPIO] Channel {global_channel_ids[1]} audio ENABLED (re-sync: PIN 40 is active)"
                    )
        ...
        for i in range(4):
            if (
                ...
                if not audio.channels[i].audio.gpio_active:
                    audio.channels[i].audio.gpio_active = 1
                    print(
                        f"[GPIO] Channel {global_channel_ids[2]} audio ENABLED (re-sync: PIN 16 is active)"
                    )
                    # Send transmit_started to server
        ...
        for i in range(4):
            if (
                ...
                if not audio.channels[i].audio.gpio_active:
                    audio.channels[i].audio.gpio_active = 1
                    print(
                        f"[GPIO] Channel {global_channel_ids[3]} audio ENABLED (re-sync: PIN 18 is active)"
                    )
                    # Send transmit_started to server
    ...
    while not global_interrupted.is_set():
        ...
        with gpio_mutex:
            ...
            if curr_val_38 != gpio_38_state and curr_val_38 != -1:
                ...
                for i in range(4):
                    if (
                        ...
                        audio.channels[i].audio.gpio_active = (
                            1 if curr_val_38 == 0 else 0
                        )
                        print(
                            f"Channel {global_channel_ids[0]} audio {'ENABLED' if audio.channels[i].audio.gpio_active else 'DISABLED'}"
                        )
                        ...
                        if curr_val_38 == 0:  # GPIO is active
                            ...
                            if udp.global_udp_socket and udp.global_server_addr:
                                if not audio.channels[i].audio.transmitting:
                                    print(
                                        f"[GPIO] Starting transmission for channel {global_channel_ids[0]} (GPIO active, UDP ready)"
                                    )
                                    # Check if key is already set
                                    if any(audio.channels[i].audio.key):
                                        if audio.start_transmission_for_channel(
                                            audio.channels[i].audio
                                        ):
                                            print(
                                                f"[GPIO] Transmission started for channel {global_channel_ids[0]}"
                                            )
                                        else:
                                            print(
                                                f"[GPIO] Failed to start transmission for channel {global_channel_ids[0]}"
                                            )
                        break

                import websocket

                websocket.send_websocket_transmit_event(
                    global_channel_ids[0], 1 if curr_val_38 == 0 else 0
                )
            ...
            if curr_val_40 != gpio_40_state and curr_val_40 != -1:
                ...
                for i in range(4):
                    if (
                        ...
                        audio.channels[i].audio.gpio_active = (
                            1 if curr_val_40 == 0 else 0
                        )
                        print(
                            f"Channel {global_channel_ids[1]} audio {'ENABLED' if audio.channels[i].audio.gpio_active else 'DISABLED'}"
                        )
                        ...
                        if curr_val_40 == 0:  # GPIO is active
                            ...
                            if udp.global_udp_socket and udp.global_server_addr:
                                if not audio.channels[i].audio.transmitting:
                                    print(
                                        f"[GPIO] Starting transmission for channel {global_channel_ids[1]} (GPIO active, UDP ready)"
                                    )
                                    # Check if key is already set
                                    if any(audio.channels[i].audio.key):
                                        if audio.start_transmission_for_channel(
                                            ...
                                            print(
                                                f"[GPIO] Transmission started for channel {global_channel_ids[1]}"
                                            )
                                        else:
                                            print(
                                                f"[GPIO] Failed to start transmission for channel {global_channel_ids[1]}"
                                            )
                        break
                ...
                websocket.send_websocket_transmit_event(
                    global_channel_ids[1], 1 if curr_val_40 == 0 else 0
                )
            ...
            if curr_val_16 != gpio_16_state and curr_val_16 != -1:
                ...
                for i in range(4):
                    if (
                        ...
                        audio.channels[i].audio.gpio_active = (
                            1 if curr_val_16 == 0 else 0
                        )
                        print(
                            f"Channel {global_channel_ids[2]} audio {'ENABLED' if audio.channels[i].audio.gpio_active else 'DISABLED'}"
                        )
                        ...
                        if curr_val_16 == 0:  # GPIO is active
                            ...
                            if udp.global_udp_socket and udp.global_server_addr:
                                if not audio.channels[i].audio.transmitting:
                                    print(
                                        f"[GPIO] Starting transmission for channel {global_channel_ids[2]} (GPIO active, UDP ready)"
                                    )
                                    # Check if key is already set
                                    if any(audio.channels[i].audio.key):
                                        if audio.start_transmission_for_channel(
                                            ...
                                            print(
                                                f"[GPIO] Transmission started for channel {global_channel_ids[2]}"
                                            )
                                        else:
                                            print(
                                                f"[GPIO] Failed to start transmission for channel {global_channel_ids[2]}"
                                            )
                        break
                ...
                websocket.send_websocket_transmit_event(
                    global_channel_ids[2], 1 if curr_val_16 == 0 else 0
                )
            ...
            if curr_val_18 != gpio_18_state and curr_val_18 != -1:
                ...
                if global_channel_count > 3:
                    for i in range(4):
                        if (
                            audio.channels[i].active
                            and audio.channels[i].audio.channel_id
                            == global_channel_ids[3]
                        ):
                            audio.channels[i].audio.gpio_active = (
                                1 if curr_val_18 == 0 else 0
                            )
                            print(
                                f"Channel {global_channel_ids[3]} audio {'ENABLED' if audio.channels[i].audio.gpio_active else 'DISABLED'}"
                            )
                            ...
                            if curr_val_18 == 0:  # GPIO is active
                                ...
                                if udp.global_udp_socket and udp.global_server_addr:
                                    if not audio.channels[i].audio.transmitting:
                                        print(
                                            f"[GPIO] Starting transmission for channel {global_channel_ids[3]} (GPIO active, UDP ready)"
                                        )
                                        # Check if key is already set
                                        if any(audio.channels[i].audio.key):
                                            if audio.start_transmission_for_channel(
                                                ...
                                                print(
                                                    f"[GPIO] Transmission started for channel {global_channel_ids[3]}"
                                                )
                                            else:
                                                print(
                                                    f"[GPIO] Failed to start transmission for channel {global_channel_ids[3]}"
                                                )
                            break
                    ...
                    websocket.send_websocket_transmit_event(
                        global_channel_ids[3], 1 if curr_val_18 == 0 else 0
                    )
            ...
            if status_counter >= 100:
                ...
                print(
                    f"PIN 38 (GPIO 20): {'ACTIVE' if curr_val_38 == 0 else 'INACTIVE'} (Channel: {global_channel_ids[0]})"
                )
                print(
                    f"PIN 40 (GPIO 21): {'ACTIVE' if curr_val_40 == 0 else 'INACTIVE'} (Channel: {global_channel_ids[1]})"
                )
                print(
                    f"PIN 16 (GPIO 23): {'ACTIVE' if curr_val_16 == 0 else 'INACTIVE'} (Channel: {global_channel_ids[2]})"
                )
                if global_channel_count > 3:
                    print(
                        f"PIN 18 (GPIO 24): {'ACTIVE' if curr_val_18 == 0 else 'INACTIVE'} (Channel: {global_channel_ids[3]})"
                    )
                else:
                    print(
                        f"PIN 18 (GPIO 24): {'ACTIVE' if curr_val_18 == 0 else 'INACTIVE'} (Channel: N/A)"
                    )
