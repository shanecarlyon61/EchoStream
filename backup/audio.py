        ...
        try:
            ...
            if device_info["maxInputChannels"] > 0:
                ...
                if "usb" in name or "audio device" in name or "headset" in name:
                    ...
                    print(
                        f"USB Device {i} assigned to slot {usb_count}: {device_info['name']}"
                    )
                    usb_count += 1
        except Exception:
            continue
    ...
    while not global_interrupted.is_set() and audio_stream.transmitting:
        if not audio_stream.gpio_active:
            ...
            if skip_count % 100 == 0:  # Log every 10 seconds
                print(
                    f"[AUDIO DEBUG] Channel {audio_stream.channel_id}: GPIO not active (gpio_active={audio_stream.gpio_active})"
                )
            time.sleep(0.1)
            continue
        ...
        if audio_stream.input_stream is None:
            ...
            if skip_count % 100 == 0:  # Log every 10 seconds
                print(
                    f"[AUDIO DEBUG] Channel {audio_stream.channel_id}: Input stream is None"
                )
            time.sleep(0.1)
            continue
        ...
        try:
            ...
            if len(data) == 0:
                ...
                if skip_count % 100 == 0:
                    print(
                        f"[AUDIO DEBUG] Channel {audio_stream.channel_id}: Read 0 bytes from input stream"
                    )
                time.sleep(0.01)
                continue
            ...
            if read_count % 1000 == 0:  # Log every ~10 seconds at 48kHz
                print(
                    f"[AUDIO DEBUG] Channel {audio_stream.channel_id}: Read {len(audio_data)} samples (read_count={read_count}, skip_count={skip_count})"
                )
            ...
            if channel_has_tone_detect and tone_detect.is_tone_detect_enabled():
                with global_shared_buffer.mutex:
                    ...
                    global_shared_buffer.samples[:sample_count] = audio_data[
                        :sample_count
                    ]
                ...
                static_buffer_write_count = getattr(
                    audio_input_worker, "_buffer_write_count", {}
                )
                ...
                if static_buffer_write_count[audio_stream.channel_id] % 1000 == 0:
                    print(
                        f"[AUDIO DEBUG] Writing audio to shared buffer for tone detection (channel {audio_stream.channel_id}, count={static_buffer_write_count[audio_stream.channel_id]})"
                    )
                    ...
                    if audio_stream.encoder:
                        ...
                        if opus_data and len(opus_data) > 0:
                            ...
                            encrypted = crypto.encrypt_data(
                                opus_data, bytes(audio_stream.key)
                            )
                            if encrypted:
                                # Base64 encode
                                ...
                                if udp.global_udp_socket and udp.global_server_addr:
                                    ...
                                    try:
                                        udp.global_udp_socket.sendto(
                                            msg.encode("utf-8"), udp.global_server_addr
                                        )

                                        # Track audio transmission for debugging
                                        static_send_count = getattr(
                                            audio_input_worker, "_send_count", {}
                                        )
                                        if (
                                            audio_stream.channel_id
                                            not in static_send_count
                                        ):
                                            static_send_count[
                                                audio_stream.channel_id
                                            ] = 0
                                        ...
                                        if (
                                            static_send_count[audio_stream.channel_id]
                                            <= 5
                                            or static_send_count[
                                                audio_stream.channel_id
                                            ]
                                            % 500
                                            == 0
                                            ...
                                            print(
                                                f"[AUDIO TX] Channel {audio_stream.channel_id}: Sent audio packet #{static_send_count[audio_stream.channel_id]} "
                                                f"({len(msg)} bytes) to {udp.global_server_addr}"
                                            )
                                    except Exception as e:
                                        print(
                                            f"[AUDIO TX ERROR] Channel {audio_stream.channel_id}: Failed to send audio: {e}"
                                        )
...
def audio_output_worker(audio_stream: AudioStream):
    ...
    is_passthrough_target = is_configured_passthrough_channel_id(
        audio_stream.channel_id
    )
    ...
    while not global_interrupted.is_set() and audio_stream.transmitting:
        if audio_stream.output_stream is None:
            ...
            if silence_count % 100 == 0:  # Log every 10 seconds
                print(
                    f"[AUDIO DEBUG] Channel {audio_stream.channel_id}: Output stream is None"
                )
            time.sleep(0.1)
            continue
        ...
        try:
            ...
            if (
                ...
                and tone_detect.global_tone_detection.passthrough_active
            ):
                ...
                if recording:
                    ...
                    with global_shared_buffer.mutex:
                        if (
                            global_shared_buffer.valid
                            and global_shared_buffer.sample_count > 0
                        ):
                            # Copy actual input audio from shared buffer to output with gain boost
                            samples_to_copy = min(
                                FRAMES_PER_BUFFER, global_shared_buffer.sample_count
                            )
                            ...
                            samples_to_play = np.zeros(
                                FRAMES_PER_BUFFER, dtype=np.float32
                            )

                            for i in range(samples_to_copy):
                                sample = (
                                    global_shared_buffer.samples[i] * passthrough_gain
                                )
                            ...
                            if samples_to_copy < FRAMES_PER_BUFFER:
                                last_sample = (
                                    global_shared_buffer.samples[samples_to_copy - 1]
                                    * passthrough_gain
                                ...
                                for i in range(samples_to_copy, FRAMES_PER_BUFFER):
                                    samples_to_play[i] = (
                                        last_sample  # Repeat last sample to avoid clicks
                                    )
            ...
            if samples_to_play is None:
                ...
                with audio_stream.output_jitter.mutex:
                    ...
                    # Track if we've ever had frames (to distinguish initial wait from underrun)
                    if not hasattr(audio_output_worker, "_has_had_frames"):
                        audio_output_worker._has_had_frames = {}
                    if (
                        audio_stream.channel_id
                        not in audio_output_worker._has_had_frames
                    ):
                        audio_output_worker._has_had_frames[audio_stream.channel_id] = (
                            False
                        )
                    ...
                    MIN_STARTUP_FRAMES = (
                        2  # Wait for at least 2 frames (40ms) before starting playback
                    )
                    if jitter_frames == 0:
                        # Buffer empty
                        if not audio_output_worker._has_had_frames.get(
                            audio_stream.channel_id, False
                        ):
                            ...
                            time.sleep(
                                0.05
                            )  # Wait 50ms for initial packets (allows 2-3 packets at 20ms intervals)
                            continue  # Skip this iteration, try again next loop
                    elif (
                        jitter_frames < MIN_STARTUP_FRAMES
                        and not audio_output_worker._has_had_frames.get(
                            audio_stream.channel_id, False
                        )
                    ):
                    ...
                    if jitter_frames == 0 and audio_output_worker._has_had_frames.get(
                        ...
                        if static_underrun_count[audio_stream.channel_id] == 1:
                            print(
                                f"[JITTER UNDERRUN] Channel {audio_stream.channel_id}: Buffer empty! (read_idx={audio_stream.output_jitter.read_index}, write_idx={audio_stream.output_jitter.write_index})"
                            )
                        elif static_underrun_count[audio_stream.channel_id] % 500 == 0:
                            print(
                                f"[JITTER UNDERRUN] Channel {audio_stream.channel_id}: Buffer still empty (underrun_count={static_underrun_count[audio_stream.channel_id]})"
                            )
                    ...
                    # IMPORTANT: Always fill entire buffer - match C implementation exactly
                    # The C implementation fills the entire buffer even if jitter buffer is empty (with silence)
                    while frames_filled < frames_to_fill:
                        if audio_stream.output_jitter.frame_count > 0:
                            frame = audio_stream.output_jitter.frames[
                            ...
                            if frame.valid:
                                ...
                                for i in range(frames_to_copy):
                                    sample = (
                                        frame.samples[
                                            audio_stream.current_output_frame_pos + i
                                        ]
                                        * output_gain
                                    )
                                    # Clamp to prevent distortion
                                ...
                                if (
                                    ...
                                    if output_count % 500 == 0:
                                        print(
                                            f"[JITTER READ] Channel {audio_stream.channel_id}: Consumed frame at idx {old_read_idx}, "
                                ...
                                static_invalid_count = getattr(
                                    audio_output_worker, "_invalid_frame_count", {}
                                )
                                ...
                                audio_output_worker._invalid_frame_count = (
                                    static_invalid_count
                                )

                                if (
                                    static_invalid_count[audio_stream.channel_id] % 100
                                    == 0
                                ):
                                    print(
                                        f"[JITTER WARNING] Channel {audio_stream.channel_id}: Invalid frame at read_idx {audio_stream.output_jitter.read_index} "
                            ...
                            # This ensures we always fill the entire buffer (C always fills, even if empty)
                    ...
                    # This matches the C implementation which always fills the output buffer
            ...
            # However, we need to ensure we don't consume frames faster than they arrive
            # The C implementation uses hardware-driven callbacks, so timing is automatic
            # For blocking I/O, we rely on PyAudio's internal buffering to rate-limit
            ...
            if samples_to_play is not None and len(samples_to_play) > 0:
                ...
                if audio_stream.channel_id not in audio_output_worker._last_write_time:
                    audio_output_worker._last_write_time[audio_stream.channel_id] = (
                        time.time()
                    )
                ...
                try:
                    ...
                    # The rate limiting above ensures we write at exactly the right rate
                    audio_stream.output_stream.write(
                        audio_bytes, exception_on_underflow=False
                    )
                    write_count += 1
                    audio_output_worker._last_write_time[audio_stream.channel_id] = (
                        current_time
                    ...
                    print(
                        f"[AUDIO ERROR] Channel {audio_stream.channel_id}: Write error: {e}"
                    )
                ...
                if output_count % 500 == 0:  # Log every ~10 seconds
                    ...
                    if hasattr(process_received_audio, "_receive_count"):
                        receive_count_dict = getattr(
                            process_received_audio, "_receive_count", {}
                        )
                        packets_received = receive_count_dict.get(
                            audio_stream.channel_id, 0
                        )

                    if is_silence:
                        print(
                            f"[AUDIO OUT] Channel {audio_stream.channel_id}: Playing SILENCE "
                            f"(RMS={rms:.6f}, jitter_frames={jitter_frames}, packets_received={packets_received}, "
                            f"read_idx={read_idx}, write_idx={write_idx})"
                        )
                    else:
                        print(
                            f"[AUDIO OUT] Channel {audio_stream.channel_id}: Playing AUDIO "
                    ...
                    if jitter_frames == 0:
                        print(
                            f"[AUDIO WARNING] Channel {audio_stream.channel_id}: Buffer EMPTY during playback! "
                            f"UDP packets_received={packets_received} - check if packets are arriving"
                        )
                    elif jitter_frames < 2:
                        print(
                            f"[AUDIO WARNING] Channel {audio_stream.channel_id}: Buffer LOW (frames={jitter_frames}) - "
                ...
                try:
                    audio_stream.output_stream.write(
                        silence.tobytes(), exception_on_underflow=False
                    )
                except Exception as e:
                    print(
                        f"[AUDIO ERROR] Channel {audio_stream.channel_id}: Silence write error: {e}"
                    )
                    time.sleep(0.01)
                    continue
                ...
                if silence_count % 1000 == 0:  # Log every ~10 seconds
                    with audio_stream.output_jitter.mutex:
                        jitter_frames = audio_stream.output_jitter.frame_count
                    print(
                        f"[AUDIO OUT] Channel {audio_stream.channel_id}: Playing silence (fallback) "
...
def start_transmission_for_channel(audio_stream: AudioStream) -> bool:
    ...
    try:
        ...
        try:
            ...
            print(
                f"Output stream failed for channel {audio_stream.channel_id}, using input-only mode: {e}"
            )
            audio_stream.output_stream = None
        ...
        input_thread = threading.Thread(
            target=audio_input_worker, args=(audio_stream,), daemon=True
        )
        input_thread.start()

        if audio_stream.output_stream:
            output_thread = threading.Thread(
                target=audio_output_worker, args=(audio_stream,), daemon=True
            )
            output_thread.start()
        ...
        print(
            f"Failed to start transmission for channel {audio_stream.channel_id}: {e}"
        )
        # Clean up on failure
        if audio_stream.input_stream:
            try:
...
def process_received_audio(
    audio_stream: AudioStream, opus_data: bytes, channel_id: str, target_index: int
):
    ...
    if (
        static_receive_count[channel_id] <= 5
        or static_receive_count[channel_id] % 500 == 0
    ):
        print(
            f"[AUDIO RX] Channel {channel_id}: Received audio packet #{static_receive_count[channel_id]} "
        ...
        if len(pcm_data) > 0:
            ...
            with audio_stream.output_jitter.mutex:
                ...
                if audio_stream.output_jitter.frame_count < JITTER_BUFFER_SIZE:
                    ...
                    static_last_packet_time = getattr(
                        process_received_audio, "_last_packet_time", {}
                    )
                    ...
                    if (
                        ...
                        print(
                            f"[JITTER WRITE] Channel {channel_id}: Added frame at idx {write_idx_before}, "
                    ...
                    if static_receive_count[channel_id] % 100 == 0:
                        print(
                            f"[JITTER OVERFLOW] Channel {channel_id}: Buffer full, dropped frame at idx {old_read_idx}, "
                ...
                if buffer_before == 0 and audio_stream.output_jitter.frame_count > 0:
                    print(
                        f"[JITTER RECOVERY] Channel {channel_id}: Buffer refilled! frames={audio_stream.output_jitter.frame_count}, "
                ...
                if static_receive_count[channel_id] % 500 == 0:
                    ...
                    print(
                        f"[JITTER STATUS] Channel {channel_id}: frames={audio_stream.output_jitter.frame_count}/"
                    ...
                    if audio_stream.output_jitter.frame_count < 3:
                        print(
                            f"[JITTER WARNING] Channel {channel_id}: Buffer consistently low (frames={audio_stream.output_jitter.frame_count}) - "
...
def set_passthrough_output_mode(passthrough_mode: bool) -> bool:
    ...
    print(
        f"[INFO] Passthrough output mode set to {'PASSTHROUGH' if passthrough_mode else 'ECHOSTREAM'}"
    )
    return True
