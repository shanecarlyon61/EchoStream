...
def add_tone_definition(
    ...
    tone_b_range: int,
    record_length: int,
    detection_tone_alert: Optional[str],
) -> bool:
...
def add_frequency_filter(
    filter_id: str, frequency: float, filter_range: int, filter_type: str
) -> bool:
...
def freq_from_fft(sig, fs=SAMPLE_RATE):
    ...
    try:
        true_i = parabolic(np.log(magnitudes + 1e-10), i)[
            0
...
def check_tone_duration(
    is_tone_b: bool, current_time_ms: int, tone_def: ToneDefinition
) -> bool:
...
def trigger_tone_passthrough(tone_def: ToneDefinition):
    ...
    print(
        f"  Duration: {tone_def.record_length_ms} ms ({tone_def.record_length_ms / 1000:.1f} seconds)"
    )
    print(
        f"  Alert Type: {tone_def.detection_tone_alert if tone_def.detection_tone_alert else 'N/A'}"
    )
    print("=" * 60)
...
def process_audio_python_approach(samples: np.ndarray, sample_count: int) -> bool:
    ...
    with global_tone_detection.mutex:
        ...
        if (
            len(global_tone_detection.audio_buffer)
            > global_tone_detection.max_buffer_samples
        ):
            global_tone_detection.audio_buffer = global_tone_detection.audio_buffer[
                -global_tone_detection.max_buffer_samples :
            ]
    ...
    if not static_start_logged:
        valid_tones = sum(
            1 for td in global_tone_detection.tone_definitions if td.valid
        )
        print(
            f"[TONE DETECTION START] Tone detection active: {global_tone_detection.active}"
        )
        ...
        for i, td in enumerate(global_tone_detection.tone_definitions):
            if td.valid:
                print(
                    f"[TONE DETECTION START] Tone {i+1}: ID={td.tone_id}, A={td.tone_a_freq}Hz±{td.tone_a_range_hz} ({td.tone_a_length_ms}ms), B={td.tone_b_freq}Hz±{td.tone_b_range_hz} ({td.tone_b_length_ms}ms)"
                )
    ...
    if (
        static_process_count % 1000 == 0
    ):  # Log every 1000th call (~every 10 seconds at 48kHz)
        valid_tones = sum(
            1 for td in global_tone_detection.tone_definitions if td.valid
        )
        buffer_seconds = buffer_len / SAMPLE_RATE
        print(
            f"[TONE DEBUG] Processing audio: {valid_tones} tone(s), buffer={buffer_seconds:.2f}s ({buffer_len} samples)"
        )
    ...
    recent_samples = np.array(
        global_tone_detection.audio_buffer[-SAMPLE_RATE:]
    )  # Last 1 second
    ...
    for l_a, l_b in unique_lengths:
        ...
        try:
            ...
            tone_a_segment = (
                buf_array[-start_idx:-end_idx]
                if end_idx > 0
                else buf_array[-start_idx:]
            )
            tone_b_segment = buf_array[-end_idx:]

            # Validate segment lengths
            if len(tone_a_segment) < int(SAMPLE_RATE * 0.1) or len(
                tone_b_segment
            ) < int(SAMPLE_RATE * 0.1):
                continue
            ...
            for tone_def in global_tone_detection.tone_definitions:
                ...
                if (
                    abs(tone_def.tone_a_length_ms / 1000.0 - l_a) > 0.1
                    or abs(tone_def.tone_b_length_ms / 1000.0 - l_b) > 0.1
                ):
                    continue

                # Check if frequencies match (use tone_a_range and tone_b_range as tolerance)
                a_match = abs(tone_def.tone_a_freq - a_tone_freq) <= max(
                    tone_def.tone_a_range_hz, tolerance
                )
                b_match = abs(tone_def.tone_b_freq - b_tone_freq) <= max(
                    tone_def.tone_b_range_hz, tolerance
                )

                # Prevent duplicate detections (like ToneDetect)
                time_since_last = (
                    current_time_ms - global_tone_detection.last_detect_time
                )
                max_tone_len_ms = max(
                    tone_def.tone_a_length_ms, tone_def.tone_b_length_ms
                )
                ...
                if a_match and b_match and time_since_last > max_tone_len_ms:
                    ...
                    print(f"  Tone ID: {tone_def.tone_id}")
                    print(
                        f"  Tone A: {a_tone_freq:.1f} Hz (target: {tone_def.tone_a_freq} Hz ±{tone_def.tone_a_range_hz} Hz)"
                    )
                    print(
                        f"  Tone B: {b_tone_freq:.1f} Hz (target: {tone_def.tone_b_freq} Hz ±{tone_def.tone_b_range_hz} Hz)"
                    )
            ...
            if static_process_count % 500 == 0:  # Log every ~5 seconds
                ...
                for tone_def in global_tone_detection.tone_definitions:
                    ...
                    if (
                        ...
                        continue
                    tolerance = max(
                        tone_def.tone_a_range_hz, tone_def.tone_b_range_hz, 10
                    )
                    ...
                    if a_match or b_match:
                        status = (
                            ...
                            else ("A_ONLY" if a_match else "B_ONLY")
                        )
                        print(
                            f"[TONE SCAN] A: {a_tone_freq:.1f}Hz, B: {b_tone_freq:.1f}Hz | "
            ...
            if static_process_count % 100 == 0:
                print(
                    f"[TONE DEBUG] Index calculation error: {e}, l_a={l_a}, l_b={l_b}"
                )
            continue
        except Exception as e:
    ...
    if global_tone_detection.recording_active:
        ...
        if remaining <= 0:
            ...
            if global_tone_detection.active_tone_def:
                ...
                print(
                    f"  Duration completed: {global_tone_detection.active_tone_def.record_length_ms} ms"
                )
            print("=" * 60)
            with global_tone_detection.mutex:
