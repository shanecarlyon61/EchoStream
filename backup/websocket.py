...
def send_websocket_transmit_event(channel_id: str, is_started: int):
    ...
    if global_ws_client is None:
        print(
            f"[WARNING] WebSocket not connected, cannot send transmit event for channel {channel_id}"
        )
        return
    ...
    try:
        ...
        print(
            f"[WEBSOCKET TX] Sending {event_type} for channel {channel_id} (PTT {'PRESSED' if is_started else 'RELEASED'})"
        )

        # Send via asyncio if we're in an async context
        if global_ws_context and global_ws_context.is_running():
            asyncio.run_coroutine_threadsafe(
                global_ws_client.send(json.dumps(transmit_msg)), global_ws_context
            )
        else:
        ...
        async with websockets.connect(ws_url) as ws:
            ...
            for i in range(4):
                if audio.channels[i].active:
                    print(
                        f"[INFO] Registering channel {audio.channels[i].audio.channel_id}"
                    )
            ...
            for i in range(4):
                if audio.channels[i].active:
                    ...
                    print(
                        f"[INFO] Sending connect message for channel {audio.channels[i].audio.channel_id}"
                    )
                    await ws.send(json.dumps(connect_msg))
                    print(
                        f"[INFO] Connect message sent successfully for channel {audio.channels[i].audio.channel_id}"
                    )
                ...
                try:
                    ...
                    if not message or len(message) == 0:
                        if message_count % 100 == 0:  # Log occasionally
                            print(
                                f"[WEBSOCKET] Received empty message (#{message_count})"
                            )
                        continue
                    ...
                    if (
                        "udp_host" in str(data)
                        and "udp_port" in str(data)
                        and "websocket_id" in str(data)
                    ):
                        print("=" * 60)
                                ...
                                for i in range(4):
                                    if audio.channels[i].active:
                                        ...
                                        if not audio.channels[i].audio.gpio_active:
                                            print(
                                                f"[WEBSOCKET] Channel {audio.channels[i].audio.channel_id}: Skipping transmission start (GPIO not active)"
                                            )
                                            continue
                                        ...
                                        if len(key_bytes) == 32:
                                            audio.channels[i].audio.key = list(
                                                key_bytes
                                            )
                                            print(
                                                f"AES key decoded for channel {audio.channels[i].audio.channel_id}"
                                            )
                                            ...
                                            if audio.start_transmission_for_channel(
                                                ...
                                                print(
                                                    f"Audio transmission started for channel {audio.channels[i].audio.channel_id} (GPIO active)"
                                                )
                                                # Send transmit_started since GPIO is already confirmed active
                                                print(
                                                    f"[UDP READY] Sending transmit_started for channel {audio.channels[i].audio.channel_id} (GPIO active, UDP now ready)"
                                                )
                                                send_websocket_transmit_event(
                                                    audio.channels[i].audio.channel_id,
                                                    1,
                                                )
                                        else:
                                            print(
                                                f"Key decode failed for channel {audio.channels[i].audio.channel_id}"
                                            )
                        ...
                        try:
                            ...
                            if isinstance(users_data, dict):
                                ...
                                if count == 1:
                                    print("=" * 60)
                                    print(
                                        "[WEBSOCKET] Users Connected Event (first occurrence):"
                                    )
                                    ...
                                    if connected_users:
                                        ...
                                        if len(connected_users) > 5:
                                            print(
                                                f"    ... and {len(connected_users) - 5} more"
                                            )
                                    ...
                                    print(
                                        f"[WEBSOCKET] 👥 Users Connected Event #{count}: {total_users} total user(s) online"
                                    )
                                    if connected_users and len(connected_users) > 0:
                                        print(
                                            f"  📡 Active connections: {len(connected_users)}"
                                        )
                        except Exception:
                            ...
                            if count == 1:
                                print("=" * 60)
                                print(
                                    "[WEBSOCKET] Users Connected Message (first occurrence):"
                                )
                                print(f"  Message: {message[:200]}")
                                print(
                                ...
                                print(
                                    f"[WEBSOCKET] 👥 Users connected message #{count} received"
                                )
                        ...
                        if udp.global_udp_socket and udp.global_server_addr:
                            if count == 1:
                                print(
                                    "[WEBSOCKET] ✅ UDP is configured and ready for audio"
                                )
                        else:
                            if count == 1:
                        ...
                        if message_count % 50 == 0:
                            print(
                                f"[WEBSOCKET] Other message (#{message_count}): {json.dumps(data, indent=2)[:300]}"
                            )
