...
def main():
    ...
    if config.load_complete_config():
        ...
        print(
            "[MAIN] Please check /home/will/.an/config.json file exists and is readable"
        )
        return 1
    ...
    for i in range(global_channel_count):
        ...
        if not audio.setup_channel(audio.channels[i], global_channel_ids[i]):
            print(
                f"Failed to setup channel {i + 1} ({global_channel_ids[i]})",
                file=sys.stderr,
            )
            return 1
        ...
        while not global_interrupted.is_set():
            try:
                ...
                with audio.global_shared_buffer.mutex:
                    ...
                    if (
                        audio.global_shared_buffer.valid
                        and audio.global_shared_buffer.sample_count > 0
                    ):
                        # Process audio for tone detection
                        samples = audio.global_shared_buffer.samples[
                            : audio.global_shared_buffer.sample_count
                        ].copy()
                        ...
                        if process_count == 0:
                            static_empty_count = getattr(
                                tone_detection_worker, "_empty_count", 0
                            )
                            tone_detection_worker._empty_count = static_empty_count + 1
                            if (
                                static_empty_count % 1000 == 0
                            ):  # Log every 1000th empty check
                                print(
                                    f"[TONE DEBUG] Shared buffer empty (valid={audio.global_shared_buffer.valid}, count={audio.global_shared_buffer.sample_count})"
                                )
    ...
    print(
        f"All {global_channel_count} channels running with single WebSocket. Press Ctrl+C to stop."
    )
    print("\n=== SYSTEM BEHAVIOR ===")
    ...
    for i in range(global_channel_count):
        ...
        print(
            "    - Output: ALWAYS plays EchoStream audio (unaffected by tone detection)"
        )
        ...
        for j in range(MAX_CHANNELS):
            ...
            if (
                channel_config
                and channel_config.valid
                and channel_config.channel_id == global_channel_ids[i]
            ):
        ...
        if has_tone_detect:
            print(
                f"    - Input: {'ENABLED' if audio.is_card1_input_enabled() else 'DISABLED'} (for tone detection and passthrough)"
            )
        else:
    ...
    print(
        f"  - Current mode: {'ENABLED' if audio.is_tone_detect_enabled() else 'DISABLED'}"
    )
