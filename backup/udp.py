...
def setup_global_udp(config: dict) -> bool:
    ...
    if global_udp_socket is not None:
        print(
            f"UDP socket already configured for {config['udp_host']}:{config['udp_port']}"
        )
        return True
    ...
    try:
        ...
        print(
            f"Global UDP socket configured for {config['udp_host']}:{config['udp_port']}"
        )
        ...
        if udp_listener_thread is None or not udp_listener_thread.is_alive():
            udp_listener_thread = threading.Thread(
                target=udp_listener_worker, daemon=True
            )
        ...
        if global_udp_socket is not None and global_server_addr is not None:
            ...
            try:
                ...
                if heartbeat_count % 60 == 0:
                    print(
                        f"Heartbeat sent to keep NAT mapping active (count: {heartbeat_count})"
                    )
            except Exception as e:
    ...
    while not global_interrupted.is_set():
        try:
            ...
            ready, _, _ = select.select(
                [global_udp_socket], [], [], 1.0
            )  # 1s timeout for interrupt checking

            if not ready:
                # Timeout - check if we should continue
                timeout_count += 1
                time_since_last = (
                    time.time() - last_packet_time if packet_count > 0 else float("inf")
                )
                ...
                if timeout_count % 10 == 0:  # Log every 10th timeout (10 seconds)
                    print(
                        f"[UDP] Timeout #{timeout_count} waiting for packets (total_packets={packet_count}, "
                    ...
                    if timeout_count == 10:  # 10 seconds
                        print(
                            f"[UDP WARNING] 10 seconds elapsed - no packets received!"
                        )
                        print(f"[UDP WARNING] Possible causes:")
                        print(
                            f"[UDP WARNING]   1. Server only sends audio when client is transmitting"
                        )
                        print(
                            f"[UDP WARNING]   2. Check GPIO is active and input audio is being captured"
                        )
                        print(
                            f"[UDP WARNING]   3. Network/firewall issue blocking UDP packets"
                        )
                        print(f"[UDP WARNING]   4. Server not sending packets yet")
                        if packet_count == 0:
                            print(
                                f"[UDP WARNING] NO packets received since startup - verify client is transmitting audio!"
                            )
                        else:
                            print(
                                f"[UDP WARNING] Last packet was {time_since_last:.1f}s ago - packets may have stopped arriving"
                            )
                continue
            ...
            if timeout_count > 0:
                print(
                    f"[UDP RX] Packet received after {timeout_count} timeouts! Packet #{packet_count} from {client_addr} "
            ...
            if packet_count <= 20:
                print(
                    f"[UDP RX] Received packet #{packet_count} from {client_addr} ({len(buffer)} bytes)"
                )
            ...
            if buffer:
                try:
                    ...
                    if msg_type == "audio":
                        static_audio_count = getattr(
                            udp_listener_worker, "_audio_count", {}
                        )
                        ...
                        if (
                            static_audio_count[channel_id] <= 5
                            or static_audio_count[channel_id] % 500 == 0
                        ):
                            print(
                                f"[UDP AUDIO] Channel {channel_id}: Received audio packet #{static_audio_count[channel_id]} "
                    ...
                    # Log non-audio messages
                        ...
                        print(
                            f"[UDP] Received {msg_type} message: channel_id={channel_id}, data_length={len(data)}"
                        )
                    ...
                    if msg_type == "audio":
                        ...
                        for i in range(MAX_CHANNELS):
                            if (
                                ...
                                and audio.channels[i].audio.channel_id == channel_id
                            ):
                        ...
                        if not target_stream:
                            ...
                            if (
                                ...
                                active_channels = [
                                    audio.channels[i].audio.channel_id
                                    for i in range(MAX_CHANNELS)
                                    if audio.channels[i].active
                                ]
                                print(
                                    f"[UDP ERROR] Channel {channel_id} not found! Active channels: {active_channels}"
                                )
                            continue
                        ...
                        if len(encrypted_data) > 0:
                            if udp_debug_enabled():
                                print(
                                    f"UDP Listener: Base64 decoded successfully ({len(encrypted_data)} bytes)"
                                )
                            ...
                            decrypted = crypto.decrypt_data(
                                encrypted_data, bytes(target_stream.key)
                            )

                            if decrypted:
                                if udp_debug_enabled():
                                    print(
                                        f"UDP Listener: Data decrypted successfully ({len(decrypted)} bytes)"
                                    )

                                # Decode Opus audio and add to jitter buffer
                                audio.process_received_audio(
                                    target_stream, decrypted, channel_id, target_index
                                )
                            else:
                                if key_is_zero and target_index >= 0:
                                    if not zero_key_warned[target_index]:
                                        print(
                                            f"UDP Listener: AES key not set for channel {channel_id}; dropping encrypted audio until key is provisioned"
                                        )
                                    ...
                                    if target_index >= 0:
                                        ...
                                        if (
                                            udp_debug_enabled()
                                            or decrypt_fail_count[target_index] == 1
                                            or decrypt_fail_count[target_index] % 50
                                            == 0
                                        ):
                                            print(
                                                f"UDP Listener: Decryption failed for channel {channel_id}"
                                            )
                                    else:
                        ...
                        if udp_debug_enabled():
                            print(
                                f"UDP Listener: Non-audio message type '{msg_type}', ignoring"
                            )
