...
def get_alert_details(cfg: Dict[str, Any], channel_id: str) -> Dict[str, Any]:
    """Extract alert_details for a specific channel from config.json"""
    result = {
        "threshold": 0.7,
        "gain": 0.4,
        "db": -20,
    ...
    try:
        ...
        for key in ("channel_one", "channel_two", "channel_three", "channel_four"):
            ...
            if alert_details:
                # Handle threshold and gain as strings that may need conversion
                threshold = alert_details.get("threshold", "0.7")
                gain = alert_details.get("gain", "0.4")
                result["threshold"] = float(threshold) if isinstance(threshold, (str, int, float)) else 0.7
                result["gain"] = float(gain) if isinstance(gain, (str, int, float)) else 0.4
                result["db"] = int(alert_details.get("db", -20))
        ...
        print(f"[CONFIG] ERROR: Failed to get alert details for {channel_id}: {e}")
    return result
...
def get_device_id_from_config() -> Optional[str]:
    """Get device unique_id from config.json file.
    ...
    try:
        ...
        print(f"[CONFIG] Error loading device ID from config: {e}")
    return None
