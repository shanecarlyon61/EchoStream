                ...
                if device_index is not None:
                    ...
                    if pa and stream:
                        try:
                            ...
                            print(
                                f"[PASSTHROUGH] Opened output stream for target channel {target_channel_id} (index {target_index}, device {device_index})"
                            )
                        except Exception as e:
                            print(
                                f"[PASSTHROUGH] WARNING: Failed to start output stream: {e}"
                            )
                            close_stream(pa, stream)
                else:
                    print(
                        f"[PASSTHROUGH] WARNING: No audio device available for channel index {target_index} (target: {target_channel_id})"
                    )
                    ...
                    try:
                        ...
                        try:
                            ...
                            if write_count <= 5 or write_count % 500 == 0:
                                print(
                                    f"[PASSTHROUGH] Wrote audio chunk #{write_count} for {source_channel_id} ({len(pcm_bytes)} bytes)"
                                )
                        except Exception as e:
                            if write_count % 100 == 0:
                                print(
